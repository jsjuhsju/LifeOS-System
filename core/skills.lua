-- LifeOS-System: Core de Habilidades (Versión Final v1.5)
-- Creado por: jsjuhsju
...
-- Función principal para añadir experiencia
function AddExperience(source, skill, amount)
    ...
    MySQL.scalar('SELECT experience FROM user_skills WHERE identifier = ? AND skill_name = ?', {
        ...
        if currentExp then
            ...
            -- Notificamos al cliente para que aplique beneficios y sonidos
            TriggerClientEvent('LifeOS:client:AddXP', source, skill, amount, newLevel)
        else
            MySQL.insert('INSERT INTO user_skills (identifier, skill_name, experience, level) VALUES (?, ?, ?, ?)', {
                identifier, skill, amount, 1
            })
            TriggerClientEvent('LifeOS:client:AddXP', source, skill, amount, 1)
        end
    end)
end

-- Obtener habilidades para la tableta
function GetPlayerSkills(source, cb)
    ...
    MySQL.query('SELECT skill_name, experience, level FROM user_skills WHERE identifier = ?', {
        ...
        local skills = {
            ['agricultura'] = { xp = 0, lvl = 1 },
            ['mineria'] = { xp = 0, lvl = 1 },
            ['resistencia'] = { xp = 0, lvl = 1 },
            ['fuerza'] = { xp = 0, lvl = 1 }
        }
